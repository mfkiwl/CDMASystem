...
function modDataStream = Transmitter(data, txParams)  
    ...
    % Allocating buffer for modulated data stream
    modDataStream = zeros(size(modData, 1) * txParams.chipLen, size(modData, 2));
    userCodes = genCodes(txParams);
    
    for iter_users = 1:txParams.numUsers
        wideSymStream = modData(:, iter_users) .* userCodes(:, iter_users)';
        modDataStream(:, iter_users) = wideSymStream(:);
    end
